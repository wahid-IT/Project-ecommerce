...
<body>
	...
	while($row = mysqli_fetch_array($result)){
		$query2 = "SELECT * FROM users WHERE id ='$row[1]'";
		$result2 = mysqli_query($conn,$query2);
		$row2 = mysqli_fetch_array($result2);
		
		$query3 = "SELECT * FROM items WHERE id ='$row[2]'";
		$result3 = mysqli_query($conn,$query3);
		$row3 = mysqli_fetch_array($result3);
	?>	
										<tr>
											<td><?php echo htmlentities($cnt);?></td>
											<td><?php if($row2 > 0) echo htmlentities($row2[1]);?></td>
											<td><?php if($row3 > 0) echo htmlentities($row3[1]);?></td>
											<td><?php echo htmlentities($row['status']);?></td>
											<td>
												<a href="#editEmployeeModal" class="edit" data-toggle="modal">
													<i class="fa fa-edit update" data-toggle="tooltip" title="Modifier"></i>
												</a>
